...
{
	// TODO hier aufräumen
	public class DeviceLightStateHandler : IRequestHandler<DeviceLightStateQuery, DeviceStateDto?>
	{
		...
		public async Task<DeviceStateDto?> Handle(DeviceLightStateQuery request, CancellationToken cancellationToken)
		{
			if (request is null)
			{
			...
			{
				//return ResponsePayload.Failed<DeviceStateDto>("Error: There is no home created at the moment.", new DeviceLightStateRequestDto());
				return null;
			}
			...
			{
				return null;
				// return ResponsePayload.Failed<DeviceStateDto>($"Error: No device found by the given deviceId {request.LightStateDto.DeviceId}", new DeviceLightStateRequestDto());
			}
			...
			// return true ?
			// 	ResponsePayload.Ok<DeviceStateDto>($"{foundDevice.Name} changed light status", request.LightStateDto) :
			// 	ResponsePayload.Fail<DeviceStateDto>($"Error: Couldn't send new light status to {foundDevice.Name}", new DeviceLightStateRequestDto());
		}
	}
}
